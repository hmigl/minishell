...
static void ft_open_out (char *str, char *prompt_line, int t)
{
	...
	if (!(access(file_name, F_OK)))
	{
		...
		if (t == 1)
			g_ms->cmd_node[g_ms->pipe]->fd_out = open (file_name, O_WRONLY | O_TRUNC);
		else
			g_ms->cmd_node[g_ms->pipe]->fd_out = open (file_name, O_WRONLY | O_APPEND);
	}
	else
		g_ms->cmd_node[g_ms->pipe]->fd_out =  open (file_name, O_RDWR | O_CREAT | O_APPEND, 0644);
	return ;
}
